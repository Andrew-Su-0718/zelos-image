...
RUN pip install -U openmim==0.3.9 && rm -rf ~/.cache/pip
RUN mim install mmengine mmdet==3.1.0 mmdet3d==1.3.0 mmsegmentation==1.1.1 imagecorruptions==1.1.2 pytorch-quantization==2.1.3 psutil==5.9.5 && rm -rf ~/.cache/pip
RUN pip install jupyter==1.0.0 && && rm -rf ~/.cache/pip
SHELL ["/bin/bash", "-c"]
